...
export const careerProfile = {
  ...
  summary: [
    'C#/.NET 기반 Windows 응용프로그램을 중심으로 대용량 검사 데이터 처리, 생산 시스템 연동, 검사 결과 조회와 가시화 구조 개선을 수행해 왔습니다.',
    '처리 지연, 반복 이슈, 조회 불편 등 사용자가 직접 겪는 문제를 코드 구조와 데이터 흐름 개선으로 줄이는 데 강점이 있습니다.',
    '대표적으로 대용량 매칭 시간을 6분 22초에서 3.5초로 단축했고, 생산 연동 구조 정리로 이슈 관리 시스템 등록 건수를 약 70% 줄였으며, 파일과 로그 중심 확인을 DB 기반 이력 조회 구조로 전환했습니다.',
  ],
};
...
export const careerCompetencies: CareerCompetency[] = [
  ...
  },
  {
    ...
    description: '파일과 로그에 흩어진 검사 결과를 DB 기반 계층 구조로 정리하고 기간과 Job 조건 검색부터 상세 결과 복원까지 이어지는 조회 기능 구현',
  },
  {
...
export const careerExperiences: CareerExperience[] = [
  {
    ...
    achievements: [
      ...
      '약 8억 원 규모 고객사 수주 요구사항에 대응해 검출 영역 표현 체계 구축',
    ],
  },
  {
    company: '비마시스',
...
export const careerProjects: CareerProject[] = [
  ...
  },
  {
    ...
    problemPoints: ['검사 결과 확인이 파일과 로그 중심으로 분산', '조건별 조회와 상세 결과 추적이 어려운 구조'],
    rolePoints: ['검사 이력 데이터 모델 설계', 'SQL Mapper 기반 접근 계층 구현과 이력 조회 UI 개발'],
    contributions: [
  ...
  },
  {
    ...
    problem: '고객사 수주를 위해 실제 검출 외곽 확인이 필요했으나, 기존 사각형 표시만으로는 판정 근거를 충분히 보여주기 어려운 구조',
    problemPoints: [
      '고객사 수주를 위해 실제 검출 외곽 확인이 필요한 상황',
      '기존 사각형 표시만으로는 판정 근거를 충분히 보여주기 어려운 구조',
    ],
    rolePoints: ['검출 영역 추출 흐름 정리와 공통 저장 구조 정의', 'C# UI에서 검출 영역 폴리곤 가시화 구현'],
    contributions: [
