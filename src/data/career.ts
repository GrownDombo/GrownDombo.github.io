...
export const careerProfile = {
  ...
  headline: '성능 병목, 시스템 연동, 검사 데이터 흐름을 개선하는 개발자',
  contacts: [
    {
      label: 'Email',
  ...
  summary: [
    'C#/.NET 기반 Windows 응용프로그램을 중심으로 대용량 검사 데이터 처리, 생산 시스템 연동, 검사 결과 조회·가시화 구조 개선을 수행해 왔습니다.',
    '처리 지연, 반복 이슈, 조회 불편 등 사용자가 직접 겪는 문제를 코드 구조와 데이터 흐름 개선으로 줄이는 데 강점이 있습니다.',
    '대표적으로 대용량 매칭 시간을 6분 22초에서 3.5초로 단축했고, 생산 연동 구조 정리로 이슈 관리 시스템 등록 건수를 약 70% 줄였으며, 파일·로그 중심 확인을 DB 기반 이력 조회 구조로 전환했습니다.',
  ],
};
...
export const careerCompetencies: CareerCompetency[] = [
  {
    ...
    description: '대용량 좌표 비교, 반복 탐색, 원격 I/O처럼 시간이 크게 늘어나는 구간 측정, 후보 선별·캐싱·실행 위치 변경으로 처리 시간 단축',
  },
  {
    title: '생산 시스템 연동 안정화',
  ...
  {
    title: '검사 데이터 구조화와 조회 흐름 개발',
    description: '파일과 로그에 흩어진 검사 결과를 DB 기반 계층 구조로 정리하고 기간·Job 조건 검색부터 상세 결과 복원까지 이어지는 조회 기능 구현',
  },
  {
    title: '검사 결과 가시화와 Windows UI 개발',
    description: 'C++ 검사 결과와 C# UI 사이의 데이터 표현 방식을 정리하고 작업자가 판정 근거를 화면에서 확인할 수 있는 가시화 기능 개발',
  },
];
...
export const careerExperiences: CareerExperience[] = [
  {
    ...
    scope: [
      ...
      '생산 시스템 연동, 검사 데이터 처리, 이력 조회, 검사 결과 가시화 기능 개발',
      '대용량 데이터 처리 병목 분석, 운영 이슈 원인 추적, 기존 기능 구조 개선',
    ],
    achievements: [
      '대용량 매칭 처리 시간 6분 22초에서 3.5초로 단축',
      '생산 연동 구조 개선 후 이슈 관리 시스템 등록 건수 약 70% 감소',
      '검사 이력 조회 구조와 검출 영역 표현 체계를 구축해 분석·확인 흐름 개선',
    ],
  },
  {
    company: '비마시스',
...
export const careerProjects: CareerProject[] = [
  {
    title: '대용량 검사 데이터 매칭 성능 최적화',
    subtitle: 'Gerber-Part ROI 매칭 성능 최적화',
    role: '병목 분석, 후보 선별 구조 설계, 캐싱 적용, 결과 호환성 검증',
    problem: '검사 준비 과정에서 대량의 좌표 데이터를 반복 비교해 데이터 규모가 커질수록 처리 시간이 급격히 증가하는 구조',
    contributions: [
      '반복 비교가 발생하는 구간을 로그와 처리 흐름 기준으로 분석',
      'Module 영역과 교차하는 후보만 먼저 선별해 비교 대상을 축소',
  ...
  {
    title: '생산 시스템 연동 안정화',
    subtitle: 'MES·SECS/GEM 생산 연동 안정화',
    role: '고객사별 연동 요구사항 구현, 이벤트 처리 구조 정리, 예외 흐름 개선',
    problem: '고객사별 생산 연동 조건과 예외 처리 누적으로 변경 영향 범위와 반복 이슈 추적 비용이 증가하는 구조',
    contributions: [
      '생산 이벤트 생성, 전송 분기, 응답 반영 책임을 분리',
  ...
  },
  {
    ...
    subtitle: '파일·로그 중심 확인을 DB 기반 조회 구조로 전환',
    role: '데이터 모델 설계, SQL Mapper 기반 접근 계층 구현, 이력 조회 UI 개발',
    problem: '검사 결과 확인이 파일과 로그 중심으로 분산되어 조건별 조회와 상세 결과 추적이 어려운 구조',
    contributions: [
      '검사 결과를 화면, 부품, 알고리즘, 영역 단위로 추적할 수 있는 데이터 구조 설계',
      'MSSQL과 SQL Mapper 기반 데이터 접근 계층을 구현해 저장·조회 책임을 분리',
      '기간과 Job 기준 검색부터 상세 결과 확인까지 이어지는 조회 UI 개발',
    ],
    results: [
      '파일·로그 중심 확인을 DB 기반 조회 구조로 전환',
      '검사 결과 추적 범위를 화면·부품·알고리즘·영역 단위까지 확장',
  ...
  },
  {
    ...
    subtitle: '사각형 표시를 실제 검출 외곽 기반 표현으로 개선',
    role: '검출 영역 추출 흐름 정리, 공통 저장 구조 정의, C# UI 가시화 구현',
    problem: '기존 사각형 표시만으로는 실제 검출 외곽과 판정 근거 확인이 어려운 구조',
    contributions: [
      '검출 이미지 기준으로 외곽선을 추출하고 좌표를 원본 이미지 기준으로 복원',
      'C++ 검사 결과와 C# UI가 함께 사용할 수 있는 공통 표현 구조 정리',
      '검사 결과 화면에서 검출 영역을 폴리곤 형태로 확인할 수 있도록 UI 적용',
    ],
    results: [
      '사각형 중심 표시를 실제 검출 외곽 기반 표현으로 개선',
      '검사 결과 확인 화면에서 판정 근거 가시성 향상',
...
export const careerAdditionalProjects: CareerProject[] = [
  {
    ...
    role: '설정 UI 확장, 입력 매칭 구조 개선, 설정 저장·검증 흐름 정리',
  ...
  },
  {
    ...
    problem: '원격 PC가 공유 폴더 파일을 직접 이동·삭제하면서 네트워크 경유 비용으로 전체 처리 시간이 길어지는 구조',
    contributions: [
...
export const careerPersonalProjects = [
  'WinForms 커스텀 컨트롤 라이브러리 공개를 통해 반복 UI 구성, 속성 기반 설정, 재사용 가능한 컴포넌트 설계 방식 정리',
  'ExcelConditionPainter로 조건별 Excel 강조 표시와 주문 통계 산출을 자동화하고, 수작업 확인 흐름을 WinForms 도구로 축소',
  'CPUMemoryStressTest에서 C++20 기반 콘솔 도구로 CPU/Memory 부하 실행, 반복 측정, JSON/CSV 결과 수집 흐름 구조화',
  'RFID Collision Search Simulator에서 Prefix 기반 태그 탐색 로직을 C++로 구현하고, 재귀·반복 방식의 탐색 결과 비교 흐름 정리',
];
